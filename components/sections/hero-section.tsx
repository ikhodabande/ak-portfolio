...
export const HeroSection = forwardRef<HTMLElement, HeroSectionProps>(({ className = "" }, ref) => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set(
        [heroDescRef.current, heroInfo1Ref.current, heroInfo2Ref.current, heroSkillsRef.current],
        { opacity: 0, y: 30 }
      )
  ...
  return (
    <header id="intro" ref={ref} className={`min-h-screen flex items-center ${className}`}>
      <div ref={containerRef} className="grid lg:grid-cols-5 gap-16 w-full">
        <div className="lg:col-span-3 space-y-8">
          <div className="space-y-2">
            <div className="text-sm text-muted-foreground font-mono tracking-wider">{t.portfolio}</div>
            <h1 ref={heroTitleRef} className="text-4xl md:text-6xl font-light tracking-tight lg:text-6xl md:min-h-[120px]">
          ...
          <div className="md:space-y-6 max-w-md">
            <p ref={heroDescRef} className="md:text-xl text-muted-foreground leading-relaxed">
              {t.frontendDeveloper}
            ...
            <div ref={heroInfo1Ref} className="flex items-center gap-4 text-sm text-muted-foreground">
